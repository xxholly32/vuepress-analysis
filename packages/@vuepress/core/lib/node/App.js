...
module.exports = class App {
  ...
  constructor(options = {}) {
    ...
    // debug sourceDir repo/docs
    logger.debug("sourceDir", this.sourceDir);

    // debug Temp directory: repo/node_modules/@vuepress/core/.temp
  ...
  resolveConfigAndInitialize() {
    // https://v1.vuepress.vuejs.org/zh/api/node.html#siteconfig
    // siteConfig为测试配置使用
    logger.debug("siteConfig: " + this.options.siteConfig);
    if (this.options.siteConfig) {
      this.siteConfig = this.options.siteConfig;
    } else {
      // 获取config.js/yml/toml文件信息
    ...
    // 加载plugin,包括webpackchain,chainMarkdown等内嵌插件插件
