...
module.exports = class DevProcess extends EventEmitter {
  ...
  async process() {
    //  加载cacheloader 默认使用的是path.resolve(__dirname, '../../node_modules/.cache/vuepress')
    this.context.resolveCacheLoaderOptions();
    // 引入 chokidar 检测 component 和 md文件 是否改变
    this.watchSourceFiles();
    // 引入 chokidar 检测用户配置文件 config.js 是否改变
    this.watchUserConfig();
    // 文件修改就做判断 TODO: 这里的事件没怎么搞懂
    this.watchFrontmatter();
    // 一个debugtip的 日志记录，没看出有什么实际意义
    this.setupDebugTip();
  ...
  watchSourceFiles() {
    ...
    this.pagesWatcher.on("addDir", target =>
      this.handleUpdate("addDir", target)
    );
    this.pagesWatcher.on("unlinkDir", target =>
      this.handleUpdate("unlinkDir", target)
    );
  }
    ...
    const serverConfig = Object.assign(
      {
        ...
        host: this.host,
        contentBase,
        // 加入了before 和 after 的 hook
        before: (app, server) => {
          if (fs.existsSync(contentBase)) {
            // 配置为静态的资源，为什么用express，webpack dev server没有配置么？
